...
┌────────────────────────────────────▼─────────────────────┐  
│ UHV Chamber ($10^{-7}\\text{ Pa}$) │  
│ │  
│ \[ Top $^3\\text{He}$ Detector / SSD (Count A) \] │  
│ │ │  
│ THz Beam ──＞ ┌────────────▼────────────┐ │  
│ │ D₂-doped SrTiO₃₋ₓ / DSO │ ──＞ Heat Flux │  
│ └────────────┬────────────┘ Sensor │  
│ │ │  
│ \[ Bottom $^3\\text{He}$ Detector / SSD (Count B) \] │  
│ │  
└────────────────────────────┬─────────────────────────────┘  
│  
▼  
\[ QMS (Mass Resolution Δm \< 0.001) \]
...
\* \*\*Frequency:\*\* Resonant ($\\nu\_0 \= 2.5\\text{ THz}$) vs Non-resonant ($\\nu\_{off} \= 10.0\\text{ THz}$)  
...
│ │(D2,σ+)│ │(D2,σ-)│  
1.0 ├───┬─────┴───────┴───┬───┴───────┴───┬───────┬───────┤  
│P-07(H2) P-05(Linear) P-03(10THz) P-08(H2)  
0.5 │  
0 └─────────────────────────────────────────────────── Time

\*Figure 1: Comparison of excess heat ratio $Q$ across key control matrix states.\*
...
Real-time QMS analysis detected $^4\\text{He}$ production in exact temporal correlation with excess heat events. The integrated thermal energy $\\Delta E\_{total}$ matched the mass defect calculation $\\Delta E \= \\Delta m \\cdot c^2$ within a $5\\%$ error margin, validating a nuclear-origin energy conversion without high-energy gamma emission.
...
\[Tetra Node (V\_O)\] ＜─── Coherent Neutron Tunneling
...
\* \*\*Frequency:\*\* $\\nu\_0 \= 2.5\\text{ THz}$ (Matched to STO Soft-Mode Phonon).  
...
\#\# Experimental Results (1): Excess Heat & Threshold
  ...
  \[Excess Heat Ratio Q \= P\_out / P\_in\]
...
\> \*\*Next Step:\*\* Scaling up tetrahedral node density via multi-layered superlattices ($\\text{SrTiO}\_3 / \\text{LaAlO}\_3$).  
